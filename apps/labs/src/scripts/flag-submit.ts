...
if (flagForm) {
  ...
  const loginButton = flagForm.querySelector<HTMLButtonElement>("[data-login]");
  const supabase = supabaseUrl && supabaseKey ? createClient(supabaseUrl, supabaseKey) : null;
  let session: { access_token: string } | null = null;
  let nickname = "";
  let feedbackTimer: number | undefined;
  const nicknameDialog = document.querySelector<HTMLDialogElement>("[data-nickname-dialog]");
  const loginDialog = document.querySelector<HTMLDialogElement>("[data-login-dialog]");
  const loginContinue = loginDialog?.querySelector<HTMLButtonElement>("[data-login-continue]");
  const loginCancel = loginDialog?.querySelector<HTMLButtonElement>("[data-login-cancel]");
  ...
  const updateAuth = (nextSession: { access_token: string } | null) => { session = nextSession; if (authState) authState.textContent = session ? "Sesión iniciada. El ranking mostrará solo tu nickname público." : "Iniciá sesión para registrar tu solve."; if (loginButton) { loginButton.textContent = session ? "Sesión iniciada con GitHub" : "Iniciar sesión con GitHub"; loginButton.disabled = Boolean(session); } };
  const openLoginDialog = () => { if (loginDialog?.showModal) loginDialog.showModal(); else loginContinue?.click(); };
  const continueWithLogin = async () => { loginDialog?.close(); if (!supabase) { if (authState) authState.textContent = "El login todavía no está configurado."; return; } const { error } = await supabase.auth.signInWithOAuth({ provider: "github", options: { redirectTo: window.location.href } }); if (error && authState) authState.textContent = "No se pudo iniciar sesión. Intentá nuevamente."; };
  loginButton?.addEventListener("click", openLoginDialog);
  loginContinue?.addEventListener("click", continueWithLogin);
